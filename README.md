...
- [x] 测试
  > 当前只针对 Slugger 对象进行了测试，具体测试文件为 [`test/slug.js`](test/slug.js)。
- [x] 语法覆盖率
  > 针对 GFM 语法规范进行检验性测试，直接严格比对，未处理单引号转换和 TAB 的差异。测试文件为 [`test/gfm.js`](test/gfm.js) 和 [`test/diff.js`](test/diff.js)，有先后顺序，测试结果为 [`test/coverage.md`](test/coverage.md)。
- [ ] 命令行支持
- [ ] 插件系统
